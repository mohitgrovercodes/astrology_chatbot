    ...
    def validate(
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as pool:
            ...
            for future in as_completed(futures):
                if live_chat and time.time() > deadline:
                    timed_out = True
                    print(f"\n  ⏱️  Timeout ({timeout_sec}s) — stopping early with partial results")
                    pool.shutdown(wait=False, cancel_futures=True)
                    break
                try:
                    idx, verdicts = future.result(timeout=max(1, deadline - time.time()))
