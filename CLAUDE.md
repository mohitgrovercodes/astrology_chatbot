...
- **Embedding dimension is `.env`-driven, not hardcoded**: every `VertexAIEmbeddings(...)` site MUST pass `output_dimensionality=settings.EMBEDDING_DIMENSIONS` (default 1536, set in `.env`). The model returns 768-d if this kwarg is omitted, which mismatches the ChromaDB index and silently breaks every query. Audited sites: `src/api/dependencies.py`, `src/rag/memory_retriever.py`, `src/rag/preprocessing/embedder.py` -- all read the same setting now.
